        ...
        for (int i = 0; i < linkages.getLength(); i++) {
            ...
            if (linkage.toLowerCase().contains("request=getrecordbyid")) {
                ...
                link.setHref(urlPattern
                        .replace("{datasetfeed-uuid}", URLEncoder.encode(linkage, StandardCharsets.UTF_8.toString()))
                        .replace("{servicefeed-uuid}", StringUtils.encodeForPath(serviceFeed.getUuid())));
                link.setHrefLang("de");
                ...
                String code = XPATH.getString(isoDoc, identifierPath + "gmd:code/gco:CharacterString|" + identifierPath + "gmd:code/gmx:Anchor");
                if (code != null) {
                ...
                for (int j = 0; j < nl.getLength(); j++) {
                    String refSystemCode = XPATH.getString(nl.item(j), "gmd:code/gco:CharacterString|gmd:code/gmx:Anchor");
