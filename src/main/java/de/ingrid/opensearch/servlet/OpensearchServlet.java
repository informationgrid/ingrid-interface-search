...
import de.ingrid.opensearch.util.OpensearchUtil;
...
public class OpensearchServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        // set timeout
        int maxSearchTimeout = OpensearchConfig.getInstance().getInt(OpensearchConfig.IBUS_SEARCH_MAX_TIMEOUT, 60000);
    ...
    private IngridQuery createIngridQueryFromRequest(RequestWrapper requestWrapper) {
        ...
        // if detail data are requested, set direct data property to get the date during search
        // this improves performance, because we do not need getRecord requests to retrieve the data
    ...
    private String[] getRequestedMetadata(RequestWrapper requestWrapper, IngridQuery query) {
        ...
        if (!OpensearchUtil.hasPositiveDataType(query, "address")) {
            ...
            if (requestWrapper.withIngridData()) {
                String[] additional = new String[] { "T01_object.obj_class",
                        "T011_obj_serv_op_connpoint.connect_point", "partner", "provider", "t1", "t2" };
                requestedMetadata = (String[]) ArrayUtils.addAll(requestedMetadata, additional);
            }
        } else {
            requestedMetadata = new String[] { "T02_address.firstname", "T02_address.lastname", "T02_address.title",
                    "T02_address.address", "T02_address.adr_id" };
            if (requestWrapper.withIngridData()) {
                String[] additional = new String[] { "T02_address.typ", "T011_obj_serv_op_connpoint.connect_point",
                        "partner", "provider", "t1", "t2" };
                requestedMetadata = (String[]) ArrayUtils.addAll(requestedMetadata, additional);
            }
        }
                        ...
                        for (int j = 0; j < length; j++) {
                            ...
                            if ((x1 + x2 + y1 + y2) != 0.0) {
                                item.addElement("georss:box").addText(
                                        OpensearchUtil.deNullify(x1.toString()) + " "
                                                + OpensearchUtil.deNullify(y1.toString()) + " "
                                                + OpensearchUtil.deNullify(x2.toString()) + " "
                                                + OpensearchUtil.deNullify(y2.toString()));
    ...
    private void addIngridData(Element item, IngridHit hit, RequestWrapper requestWrapper) {
        if (requestWrapper.withIngridData()) {
            IngridHitDetail detail = (IngridHitDetail) hit.getHitDetail();
            ...
            if (requestWrapper.getMetadataDetail() && !requestWrapper.getMetadataDetailAsXMLDoc()) {
                Element idfDataNode = null;
                try {
                    PlugDescription plugDescription = bus.getIPlug(plugId);
                    if (IPlugVersionInspector.getIPlugVersion(plugDescription).equals(
                            IPlugVersionInspector.VERSION_IDF_1_0_DSC_OBJECT)) {
                        // add IDF data
    ...
    private void addItemLink(Element item, IngridHit hit, RequestWrapper requestWrapper) {
        ...
        if (url == null) {
            ...
            String qStr;
            try {
                qStr = URLEncoder.encode(getDetailQueryString(plugId, docUuid), "UTF-8");
            } catch (UnsupportedEncodingException e) { 
                log.error("Error encoding query.", e);
                throw new RuntimeException(e);
            }
            
            if (!requestWrapper.getMetadataDetailAsXMLDoc() && metadataDetailsUrl != null
                    && metadataDetailsUrl.length() > 0) {
                ...
                itemLink = proxyurl.concat("/query").concat("?q=").concat(qStr).concat("&docid=").concat(docId).concat(
                "&docuuid=").concat(docUuid).concat("&detail=1&ingrid=1");
            } else {
                itemLink = requestWrapper.getRequest().getRequestURL().substring(0,
                        requestWrapper.getRequest().getRequestURL().lastIndexOf("/")).concat("/query")
                        .concat("?q=").concat(qStr).concat("&docid=").concat(docId).concat(
                                "&docuuid=").concat(docUuid).concat("&detail=1&ingrid=1");
            }
    ...
    private String getDetailQueryString(String plugId, String docUuid) {
    ...
    private void addItemTitle(Element item, IngridHit hit, RequestWrapper requestWrapper) {
        String plugId = hit.getPlugId();
        IngridHitDetail detail = (IngridHitDetail) hit.getHitDetail();
        PlugDescription plugDescription = bus.getIPlug(plugId);
