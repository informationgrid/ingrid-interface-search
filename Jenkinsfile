pipeline {
    ...
    stages {
        ...
        stage ('Build RPM') {
            ...
            steps {
                script {
                    sh "sed -i 's/^Version:.*/Version: ${determineRpmVersion()}/' rpm/ingrid-interface-search.spec"
                    sh "sed -i 's/^Release:.*/Release: ${determineRpmReleasePart()}/' rpm/ingrid-interface-search.spec"

                    // Prepare build
                    sh "mkdir -p ./target/rpms /root/rpmbuild/SPECS"
                    sh """
                        cp ${WORKSPACE}/rpm/ingrid-interface-search.spec /root/rpmbuild/SPECS/ingrid-interface-search.spec &&
                        rpmbuild -bb /root/rpmbuild/SPECS/ingrid-interface-search.spec
                    """
                    ...
                    withCredentials([
                        ...
                        archiveArtifacts artifacts: 'target/rpms/ingrid/ingrid-interface-search-*.rpm', fingerprint: true
                    }
                    ...
                    withCredentials([
                        ...
                        archiveArtifacts artifacts: 'target/rpms/itzbund/ingrid-interface-search-*.rpm', fingerprint: true
                    }
                }
            }
        }
        ...
        stage('Deploy RPM') {
            ...
            steps {
                script {
                    ...
                    sh "mv target/bom.json target/ingrid-interface-search-${determineRpmVersion()}.bom.json"
